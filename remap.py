...
def g712(self, **words):
    ...
    l = float(words['k'])
    h = float(words['i'])
    offset = float(words['j'])
    only_finishing_cut = int(words['s'])
    quantity = int(words['l'])
    tool = int(words['t'])
        ...
        if flag_executed :
            ...
            if only_finishing_cut==0 :
                ...
                while COORDx0 - P[i][1] >= d :
                    ...
                    self.execute("G0 X%f Z%f" % ((COORDx0+bounce),(COORDz0+bounce)),lineno())
                    self.execute("G0 Z%f" % (coordZ_start),lineno())
