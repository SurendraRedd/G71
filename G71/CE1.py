...
#--------------------------------------------------                
    ...
    def preview(self,) :
        ...
        if self.abs_inc.get():
            self.z += (float(self.st_Z.get()) - self.old_st_Z) 
            self.x += (float(self.st_X.get()) - self.old_st_X)   
        else:
            self.z = float(self.st_Z.get()) + 325            
            self.x = float(self.st_X.get()) + 250 
        
        #запоминаем инкремент 
        self.old_st_Z = float(self.st_Z.get())             
        self.old_st_X = float(self.st_X.get())             
        ...
        try:
            ...
            if (ix1>self.x_old):#XXX когда ix1 == self.x_old (и ниже)
                ...
                if (iz1<self.z_old):
                    ...
                    print '4s'                
                    self.st_arc1  = 180 + degrees(atan2(self.x_old-ix1,iz1-self.z_old))
            ...
            if (ix1>self.x):
                if (iz1<self.z):
                    ...
                    self.end_arc1 = 180 - degrees(atan2(ix1-self.x,iz1-self.z)) 
            else:
                if (iz1<self.z):
                    self.end_arc1 = 360 - degrees(atan2(self.x-ix1,self.z-iz1))
                else:
            ...
            if (ix2>self.x_old):
                ...
                if (iz2<self.z_old):
                    ...
                    print '4s'
                    self.st_arc2  = 180 + degrees(atan2(self.x_old-ix2,iz2-self.z_old))
            ...
            if (ix2>self.x):
                if (iz2<self.z):
                    ...
                    self.end_arc2 = 180 - degrees(atan2(ix2-self.x,iz2-self.z))                  
            else:
                if (iz2<self.z):
                    self.end_arc2 = 360 - degrees(atan2(self.x-ix2,self.z-iz2))      
                else:
            ...
            if self.arc_g2_g3.get():
                END1 = 360 - self.st_arc1  + self.end_arc1
    ...
    def add_arc(self,A): 
        if self.fset == 3:
            ...
            geo=[ga,(self.z_old),(self.x_old),(self.z),(self.x),(float(self.rad.get())),ik]
            self.All_geo.append(geo)            
    ...
    def computation(self,p) :
        print 'p=' , p
        # [ga,(self.z_old-325),(self.x_old-250),(self.z-325),(self.x-250),(float(self.rad.get())),ik]
        ...
        if ik==1:
            ix1,iz1,ix2,iz2 = self.insc_arc_arc(x_old,z_old,x,z,r)

            print 'ik1'
            if self.debug: 
                print 'ix1 =',ix1-250, 'iz1=',iz1-325
                print 'x =' , x                      
                print 'z =' , z
                print 'oldx =' , x_old                     
                print 'oldz =' , z_old           
            ...
            if (ix1>x_old):#XXX когда ix1 == x_old (и ниже)
                if (iz1<z_old):               
                    st_arc1  = degrees(atan2(ix1-x_old,z_old-iz1)) 
                else:                               
                    st_arc1  = 180 - degrees(atan2(ix1-x_old,iz1-z_old))                    
            else:
                if (iz1<z_old):                
                    st_arc1  = 360 - degrees(atan2(x_old-ix1,z_old-iz1))                    
                else:               
                    st_arc1  = 180 + degrees(atan2(x_old-ix1,iz1-z_old))

                                     
            # end point x z
            if (ix1>x):
                if (iz1<z):
                    end_arc1 = degrees(atan2(ix1-x,z-iz1))
                else:                
                    end_arc1 = 180 - degrees(atan2(ix1-x,iz1-z)) 
            else:
                if (iz1<z):
                    end_arc1 = 360 - degrees(atan2(x-ix1,z-iz1))
                else:
                    end_arc1 = 180 + degrees(atan2(x-ix1,iz1-z))

            
            if p[0]==3:
                END1 =   -(st_arc1 + 360 - end_arc1)
            elif p[0]==2 :           
                END1 = 360 - st_arc1  + end_arc1

            self.canvas.create_arc([iz1-r,ix1-r],[iz1+r,ix1+r],
            style=ARC,outline="blue",width=2,start=st_arc1,extent=END1,stipple=None) 
        ...
        if ik==2:
            ...
            if (ix2>x_old):#XXX когда ix2 == x_old 
                if (iz2<z_old):               
                    st_arc1  = degrees(atan2(ix2-x_old,z_old-iz2)) 
                else:                               
                    st_arc1  = 180 - degrees(atan2(ix2-x_old,iz2-z_old))                    
            else:
                if (iz2<z_old):               
                    st_arc1  = 360 - degrees(atan2(x_old-ix2,z_old-iz2))                    
                else:              
                    st_arc1  = 180 + degrees(atan2(x_old-ix2,iz2-z_old))

                                     
            # end point x z
            if (ix2>x):
                if (iz2<z):
                    end_arc1 = degrees(atan2(ix2-x,z-iz2))
                else:                
                    end_arc1 = 180 - degrees(atan2(ix2-x,iz2-z)) 
            else:
                if (iz2<z):
                    end_arc1 = 360 - degrees(atan2(x-ix2,z-iz2))
                else:
                    end_arc1 = 180 + degrees(atan2(x-ix2,iz2-z))
            ...
            self.canvas.create_arc([iz2-r,ix2-r],[iz2+r,ix2+r],
            style=ARC,outline="blue",width=2,start=st_arc1,extent=END1,stipple=None)          
    ...
    def preview_C(self):
        ...
        #сбрасываем в 0 для каждой новой дуги(для inc)        
        self.old_st_Z = 0             
        self.old_st_X = 0
